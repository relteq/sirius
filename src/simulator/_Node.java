...
public class _Node extends Node implements AuroraComponent {

	// network references
	private _Link [] output_link;
	private _Link [] input_link;
	
    private Float3DMatrix sampledSRprofile;
    private Float3DMatrix splitratio;
    ...
    // used in update()
    private Float [][] inDemand;			// [nIn][nTypes]
    private float [] outSupply;				// [nOut]
    private float [] outDemandKnown;		// [nOut]
    private float [] dsratio;				// [nOut]
    private Float [][] outFlow; 			// [nOut][nTypes]
    private boolean [][] iscontributor;		// [nIn][nOut]
    private ArrayList<Integer> unknownind = new ArrayList<Integer>();		// [unknown splits]
    private ArrayList<Float> unknown_dsratio = new ArrayList<Float>();		// [unknown splits]	
    ...
    private ArrayList<Float> sendtoeach = new ArrayList<Float>();			// [min unknown splits]
    
	/////////////////////////////////////////////////////////////////////
	// getters and setters 
	/////////////////////////////////////////////////////////////////////
    
    public void setSampledSRProfile(Float3DMatrix s){
    	sampledSRprofile = s;
    }

    public void setHasSRprofile(boolean hasSRprofile) {
		if(!istrivialsplit){
	...
	private void computeLinkFlows(){
    	...
    	for(i=0;i<splitratio.n1;i++){
        	for(j=0;j<splitratio.n2;j++){
    ...
    private void resolveUnassignedSplits_A(){
    	...
    	for(i=0;i<nIn;i++){
	        for(k=0;k<Utils.numVehicleTypes;k++){
	            ...
	            // distribute remaining splits proportionally to capacities
	            if(remainingSplit>0){
	...
	public void initialize() {
		...
		if(getOutputs()!=null){
			...
			for(int i=0;i<nOut;i++){
				Output output = getOutputs().getOutput().get(i);
		...
		if(getInputs()!=null){
			...
			for(int i=0;i<nIn;i++){
				Input input = getInputs().getInput().get(i);
